...
class PlanetaryBacktester:
    """
    Planetary aspect backtesting with two-phase strategy:
    1. Approaching phase: Compare price at orb entry vs exact conjunction
    2. Separating phase: Compare price at exact conjunction vs orb exit
    """

    DEFAULT_ASPECTS = ['conjunction', 'trine', 'square', 'opposition']

    def __init__(self):
        self.conn = get_db_connection()

    def run_planetary_backtest(self, request: PlanetaryBacktestRequest) -> PlanetaryBacktestResponse:
        """Execute planetary backtesting with two-phase strategy"""
        start_time = datetime.now()
        request_id = f"{request.symbol}_planetary_{start_time.strftime('%Y%m%d_%H%M%S')}"

        # Use default aspects if none specified
        aspect_types = request.aspect_types if request.aspect_types else self.DEFAULT_ASPECTS

        logger.info(f"🪐 Starting planetary backtest: {request.planet1}-{request.planet2} for {request.symbol}")
        logger.info(f"   📊 Aspects: {aspect_types}, Orb: {request.orb_size}°")
        logger.info(f"   📅 Period: {request.start_date} to {request.end_date}")

        # Find all planetary aspects in the date range
        aspects_data = self._find_planetary_aspects(
            request.planet1, request.planet2, aspect_types,
            request.start_date, request.end_date, request.orb_size
        )

        logger.info(f"🔍 Found {len(aspects_data)} aspect periods to backtest")

        # Run backtesting for each phase
        approaching_results = []
        separating_results = []

        for aspect in aspects_data:
            # Phase 1: Approaching (orb entry -> exact)
            approaching_result = self._backtest_approaching_phase(
                request.symbol, aspect, request.orb_size
            )
            if approaching_result:
                approaching_results.append(approaching_result)

            # Phase 2: Separating (exact -> orb exit)
            separating_result = self._backtest_separating_phase(
                request.symbol, aspect, request.orb_size
            )
            if separating_result:
                separating_results.append(separating_result)

        # Calculate summary statistics
        summary_stats = self._calculate_summary_stats(approaching_results, separating_results)

        execution_time = (datetime.now() - start_time).total_seconds()

        logger.info(f"✅ Backtest completed: {len(approaching_results)} approaching, {len(separating_results)} separating trades")

        return PlanetaryBacktestResponse(
            ...
            total_aspects_found=len(aspects_data),
            approaching_phase_results=approaching_results,
            separating_phase_results=separating_results,
            summary_stats=summary_stats,
            execution_time_seconds=execution_time
        )

    def _find_planetary_aspects(self, planet1: str, planet2: str, aspect_types: List[str],
                               start_date: str, end_date: str, orb_size: float) -> List[Dict]:
        """Find all planetary aspects in the given date range with orb timing"""
        cursor = self.conn.cursor()

        # Query for planetary aspects with orb information
        cursor.execute("""
            SELECT DISTINCT
                trade_date,
                aspect_type,
                orb,
                (planet1 = %s AND planet2 = %s) OR (planet1 = %s AND planet2 = %s) as is_target_aspect
            FROM daily_planetary_aspects
            WHERE trade_date BETWEEN %s AND %s
            AND aspect_type = ANY(%s)
            AND ((planet1 = %s AND planet2 = %s) OR (planet1 = %s AND planet2 = %s))
            AND orb <= %s
            ORDER BY trade_date
        """, (planet1, planet2, planet2, planet1, start_date, end_date, aspect_types,
              planet1, planet2, planet2, planet1, orb_size))

        raw_data = cursor.fetchall()
        cursor.close()

        # Group consecutive dates into aspect periods
        aspect_periods = []
        current_period = None

        for trade_date, aspect_type, orb, is_target in raw_data:
            if not is_target:
                continue

            if current_period is None or (
                current_period['aspect_type'] != aspect_type or
                (trade_date - current_period['dates'][-1]).days > 3
            ):
                # Start new period
                if current_period:
                    aspect_periods.append(self._finalize_aspect_period(current_period))
                ...
                current_period['dates'].append(trade_date)
                current_period['orbs'].append(float(orb))

        # Finalize last period
        if current_period:
            aspect_periods.append(self._finalize_aspect_period(current_period))

        logger.info(f"📊 Consolidated {len(raw_data)} aspect dates into {len(aspect_periods)} periods")
        return aspect_periods

    def _finalize_aspect_period(self, period: Dict) -> Dict:
        """Calculate exact date and orb entry/exit for an aspect period"""
        dates = period['dates']
        orbs = period['orbs']

        # Find exact conjunction (minimum orb)
        min_orb_idx = orbs.index(min(orbs))
        exact_date = dates[min_orb_idx]

        # Find orb entry (first date) and exit (last date)
        orb_entry = dates[0]
        orb_exit = dates[-1]

        return {
            'aspect_type': period['aspect_type'],
            'orb_entry_date': orb_entry,
            'exact_date': exact_date,
            'orb_exit_date': orb_exit,
            'min_orb': min(orbs),
            'max_orb': max(orbs),
            'duration_days': (orb_exit - orb_entry).days + 1
        }

    def _backtest_approaching_phase(self, symbol: str, aspect: Dict, orb_size: float) -> Optional[PlanetaryBacktestResult]:
        """Backtest approaching phase: orb entry -> exact conjunction"""
        entry_date = aspect['orb_entry_date']
        exit_date = aspect['exact_date']

        if entry_date == exit_date:
            return None  # Skip single-day aspects

        entry_price = self._get_price_for_date(symbol, entry_date)
        exit_price = self._get_price_for_date(symbol, exit_date)
        ...
        return PlanetaryBacktestResult(
            phase="approaching",
            ...
            aspect_info={
                ...
                'min_orb': aspect['min_orb'],
                'max_orb': aspect['max_orb'],
                'duration_days': aspect['duration_days']
            }
        )

    def _backtest_separating_phase(self, symbol: str, aspect: Dict, orb_size: float) -> Optional[PlanetaryBacktestResult]:
        """Backtest separating phase: exact conjunction -> orb exit"""
        entry_date = aspect['exact_date']
        exit_date = aspect['orb_exit_date']
    ...
    def _get_price_for_date(self, symbol: str, date) -> Optional[float]:
        ...
        # If no exact match, try nearest trading day (within 3 days)
        cursor.execute("""
            SELECT close_price, trade_date
            FROM market_data
            WHERE symbol = %s
            AND trade_date BETWEEN %s AND %s
            ORDER BY ABS(EXTRACT(epoch FROM (trade_date - %s)))
            LIMIT 1
        """, (symbol, date - timedelta(days=3), date + timedelta(days=3), date))

        result = cursor.fetchone()
        cursor.close()

        if result:
            logger.debug(f"📊 Using nearby price for {symbol} on {date}: {result[1]} -> {result[0]}")
            return float(result[0])

        logger.warning(f"❌ No price data found for {symbol} near {date}")
        return None

    def _calculate_summary_stats(self, approaching_results: List[PlanetaryBacktestResult],
                                separating_results: List[PlanetaryBacktestResult]) -> Dict[str, Any]:
        """Calculate summary statistics for both phases"""
        def calc_phase_stats(results: List[PlanetaryBacktestResult], phase_name: str) -> Dict:
            if not results:
                return {
                    'total_trades': 0,
                    'avg_return_pct': 0.0,
        ...
        approaching_stats = calc_phase_stats(approaching_results, 'approaching')
        separating_stats = calc_phase_stats(separating_results, 'separating')

        # Overall comparison
        all_results = approaching_results + separating_results
        total_trades = len(all_results)

        return {
            'approaching_phase': approaching_stats,
            'separating_phase': separating_stats,
            'comparison': {
                'total_trades': total_trades,
                'approaching_outperforms': approaching_stats['avg_return_pct'] > separating_stats['avg_return_pct'],
                'return_difference': approaching_stats['avg_return_pct'] - separating_stats['avg_return_pct'],
                'best_phase': 'approaching' if approaching_stats['avg_return_pct'] > separating_stats['avg_return_pct'] else 'separating'
            }
        }
...
async def root():
    return {
        ...
        "status": "running",
        "version": "1.0.0",
        "features": ["lunar_pattern_matching", "planetary_backtesting", "astro_forecasting", "ai_recommendations"],
...
@app.post("/planetary-backtest", response_model=PlanetaryBacktestResponse)
async def run_planetary_backtest(request: PlanetaryBacktestRequest):
    """
    Run planetary aspect backtesting with two-phase strategy

    Supports configurable planetary combinations (e.g., Jupiter/Mars) and aspect types.
    If aspect_types is not specified, uses all major classical aspects:
    conjunction, trine, square, opposition.

    The two-phase strategy compares:
    1. Approaching phase: Price at orb entry vs exact conjunction
    2. Separating phase: Price at exact conjunction vs orb exit
    """

    start_time = datetime.now()
    logger.info(f"🪐 Received planetary backtest request: {request.planet1}-{request.planet2} for {request.symbol}")

    try:
        # Initialize planetary backtester
        ...
        # Validate orb size
        if request.orb_size <= 0 or request.orb_size > 20:
            raise HTTPException(
                status_code=400,
                detail="orb_size must be between 0 and 20 degrees"
            )

        # Validate planets (basic validation)
        valid_planets = ['jupiter', 'mars', 'saturn', 'venus', 'mercury', 'sun', 'moon', 'pluto', 'neptune', 'uranus']
        if request.planet1.lower() not in valid_planets or request.planet2.lower() not in valid_planets:
            raise HTTPException(
                status_code=400,
                detail=f"Invalid planet names. Valid options: {', '.join(valid_planets)}"
            )

        # Normalize planet names to lowercase
        normalized_request = PlanetaryBacktestRequest(
            symbol=request.symbol,
            market_name=request.market_name,
            planet1=request.planet1.lower(),
            planet2=request.planet2.lower(),
        ...
        # Run the backtest
        result = backtester.run_planetary_backtest(normalized_request)

        execution_time = (datetime.now() - start_time).total_seconds()
        logger.info(f"✅ Planetary backtest completed in {execution_time:.2f}s")
        ...
        logger.error(f"❌ Planetary backtest failed: {e}", exc_info=True)
