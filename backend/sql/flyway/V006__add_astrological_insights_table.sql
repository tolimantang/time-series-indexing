-- Add astrological_insights table for planetary aspect backtesting results
-- Migration: V006
-- Date: 2024-10-27
-- Description: Create table to store pre-computed planetary aspect backtesting results
...
COMMENT ON COLUMN astrological_insights.market_symbol IS 'Market identifier matching lunar_patterns format';
COMMENT ON COLUMN astrological_insights.backtest_version IS 'Version of backtesting algorithm used';

-- Create trigger to automatically update updated_at timestamp
CREATE OR REPLACE FUNCTION update_astrological_insights_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER update_astrological_insights_updated_at
    ...
    FOR EACH ROW EXECUTE FUNCTION update_astrological_insights_updated_at();